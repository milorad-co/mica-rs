...
fn pressed_abs(x: i32, y: i32, width: i32, height: i32, e: &sdl2::EventPump) -> bool {
    if e.mouse_state().left() {
        println!("X > x - (w / 2): {}", mouse_x_abs(e) > x - (width / 2));
        println!("X < w: {}", mouse_x_abs(e) < width);
        println!("Y > y - (h / 2): {}", mouse_y_abs(e) > y - (height / 2));
        println!("Y < h: {}" , mouse_y_abs(e) < height);
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // font data
    let font = ttf_context.load_font(format!("{}/mica_assets/verdana.ttf", exe_dir), 16)?;
    // enter loop
    'running: loop {
        // handle events
        ...
        if room == "image" {
            if !loaded_textures {
                textures = vec![
                    ...
                    texture_creator.load_texture(format!("{}/mica_assets/oneshot.png", exe_dir))?,
                    texture_creator.load_texture(format!("{}/mica_assets/oneshot.png", exe_dir))?,
                    texture_creator.load_texture(format!("{}/mica_assets/oneshot.png", exe_dir))?,
                ];
                sprites = vec![
            ...
            positions = vec![
                Point::new(width_abs + 832, height_abs + 342),
                Point::new(width_abs, height_abs),
                Point::new(width_abs, height_abs + 32),
                Point::new(width_abs, height_abs + 64),
                Point::new(width_abs, height_abs + 64),
                Point::new(width_abs, height_abs + 64),
            ];
            if event_pump.mouse_state().left() {
                ...
                if pressed_abs(45, 835, 300, 25, &event_pump) {
                    selected = 3;
                }
                if pressed_abs(45, 860, 300, 25, &event_pump) {
                    selected = 4;
                }
                if pressed_abs(45, 885, 300, 25, &event_pump) {
                    selected = 5;
                }
                println!("X: {}, Y: {}", mouse_x_abs(&event_pump), mouse_y_abs(&event_pump));
            ...
            if selected != 0 && event_pump.mouse_state().left() {
                if selected == 1 {
                    if lastdrawnx != -1 && lastdrawny != -1 {
                        ...
                        for _ in 0..1600 {
                            x += xstep;
                            y += ystep;
                            let surfrect = Rect::from_center(Point::new(x as i32, y as i32), (1 * size) as u32, (1 * size) as u32);
                        ...
                        let surfrect = Rect::from_center(Point::new(mouse_x_abs(&event_pump) - 32, mouse_y_abs(&event_pump)), (1 * size) as u32, (1 * size) as u32);
                        surface.fill_rect(Some(surfrect), Color::RGBA(0, 0, 0, 255))?;
                        lastdrawnx = mouse_x_abs(&event_pump) - 32;
                        lastdrawny = mouse_y_abs(&event_pump);
                    }
                ...
                if selected == 3 || selected == 4 || selected == 5 {
                    sliders[selected - 3] = Point::new(mouse_x(&mut canvas, &event_pump), 342 + (25 * (selected as i32 - 3)));
                }
            } else {
                lastdrawnx = -1;
                lastdrawny = -1;
        ...
        // font surface
        let fontsurf = font.render(&format!("Brush Size: {}", size))
            .blended(Color::RGBA(0, 0, 0, 255))?;
        let fonttexture = fontsurf.as_texture(&texture_creator)?;
        let texture = surface.as_texture(&texture_creator)?;
        // render it
        canvas.copy(&texture, None, Some(Rect::new(32, 0, 1600, 800)))?;
        canvas.copy(&fonttexture, None, Some(Rect::new(0, height as i32 - 32, 112, 32)))?;
